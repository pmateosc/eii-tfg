...
while t<=Tmax
    ...
    subplot(2,2,1); title('Sin control');
    quiver(XV0(:,1),XV0(:,2),XV0(:,3),XV0(:,4));
...
figure(2); hold on;
plot(linspace(0,20,length(max0)),max0);
plot(linspace(0,20,length(max_arbor)),max_arbor);
plot(linspace(0,20,length(max_trelat1)),max_trelat1);
plot(linspace(0,20,length(max_trelat2)),max_trelat2);
title('Velocidad de consenso');
legend('max sin control','max arbor','max trelat1','max trelat2');
figure(3); hold on;
plot(max0(99:end));
plot(max_arbor(99:end));
plot(max_trelat1(99:end));
plot(max_trelat2(99:end));
title('Velocidad de consenso ampliada');
legend('max sin control','max arbor','max trelat1','max trelat2');

save("comparativa2_b075.mat")